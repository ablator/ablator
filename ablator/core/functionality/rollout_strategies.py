...
def get_rollout_strategy(context: "WhichContext"):
    possible_rollout_strategy = (
        context.functionality.rolloutstrategy_set.filter(start_at__lt=timezone.now(), tag__in=context.client_user.tag_set.all())
        .order_by("priority")
        .first()
    )
    ...
    if not possible_rollout_strategy:
        possible_rollout_strategy = (
            context.functionality.rolloutstrategy_set.filter(start_at__lt=timezone.now()).order_by("priority").first()
        )
...
def check_roll_out_enable_globally(context: "WhichContext") -> Optional[Availability]:
    if context.rollout_strategy.strategy == RolloutStrategy.ENABLE_GLOBALLY:
        return Availability(flavor=context.rollout_strategy.possible_flavors.first(), is_enabled=True)
