...
def get_availability(context: "WhichContext"):
    try:
        context.availability = Availability.objects.select_related("flavor").get(
            flavor__in=context.functionality.flavor_set.all(), user=context.client_user
        )
...
def get_enabled_count(context: "WhichContext"):
    context.enabled_count = Availability.objects.filter(flavor__functionality=context.functionality, is_enabled=True).count()
